...
class PostRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function ratedHighest($c)
    {
        ...
        $qb = $this->createQueryBuilder('p');
        $qb->select('p,avg(r.rating) as ar')->leftJoin('p.rates', 'r')->groupBy('r.post')->orderBy(
            'ar',
            'DESC'
        )->setMaxResults($c);
        $query = $qb->getQuery();
    ...
    {
        $qb = $this->createQueryBuilder('p');
        $qb->update('AMH\MybLogBundle\Entity\Blog\Post', 'p')->set('p.visits', 'p.visits+1')->where(
            $qb->expr()->eq('p.author', $user->getId())
        );
        $qb->getQuery()->execute();
    }
}
